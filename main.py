...
# ---- Register with email (step 1: create account, step 2: verify via OTP) ----
...
async def register_email(request: RegisterRequest, db: Session = Depends(get_db)):
    ...
    Creates unverified account and sends OTP to that email.
    """
    ...
    return {"status": "otp_sent", "message": "OTP sent to your email. Please verify to complete registration."}


# ---- Send OTP for email login authorization ----
@app.post("/send-otp")
async def send_otp(request: SendOTPRequest, db: Session = Depends(get_db)):
    ...
    if purpose == "login":
        ...
        if not user:
            ...
            return {"status": "otp_sent", "message": "If this email is registered, an OTP has been sent."}
    # Rate-limit: check OTPs sent in last 1 minute
...
async def verify_otp(request: VerifyOTPRequest, db: Session = Depends(get_db)):
    ...
    if purpose == "register":
        ...
        if user:
            ...
            db.commit()
        return {"status": "verified", "message": "Email verified. Registration complete.", "user": email}
    elif purpose == "login":
        user = db.query(User).filter(User.email == email).first()
        if not user or not user.email_verified:
            raise HTTPException(status_code=401, detail="Email not registered or not verified.")
        return {"status": "success", "user": user.userid, "id": user.id}


# ---- Email + password login (for verified email accounts) ----
